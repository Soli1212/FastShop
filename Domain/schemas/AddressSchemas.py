...
class NewAddress(BaseModel):
    ...
    postal_code: str = Field(
        ..., max_length=10, description="Postal code for the address"
    )
    ...
    def validate_required_fields(cls, value):
        if not value or len(value.strip()) == 0:
            raise HTTPException(
                status_code=422, detail="This field is required and cannot be empty."
            )
        return value
    ...
    def validate_postal_code(cls, value):
        if value and len(value) > 10:
            raise HTTPException(
                status_code=422, detail="Postal code cannot exceed 10 characters."
            )
        return value
    ...
    def validate_recipient_phone(cls, value):
        if not match(r"^09\d{9}$", value):
            raise HTTPException(
                status_code=422,
                detail="Invalid phone number format. Must be a valid Iranian phone number.",
            )
        return value
...
class UpdateAddress(BaseModel):
    province: Optional[str] = Field(
        None, max_length=100, description="Name of the province"
    )
